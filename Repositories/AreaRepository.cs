...
namespace Repositories
{
    public class AreaRepository : IAreaRepository
    {
        ...
        public List<Area> SearchArea(string searchString, int page, int pageSize)
        {
            try
            {
                var list = _cotamContext.Areas
                        .Where(x => x.Name.Contains(searchString)
                        || x.District.Contains(searchString)).Skip((page - 1) * (int)pageSize)
                        .Take((int)pageSize).ToList();
                if (list != null)
                {
                    return list;
        ...
        public int CountAreasWhenSearch(string searchString)
        {
            var count = _cotamContext.Areas
                        .Where(x => x.Name.Contains(searchString)
                        || x.District.Contains(searchString)).Count();
            return count;
        }
    }
}
